...
function insertStatement(table, columns, row) {
  ...
  return `INSERT OR IGNORE INTO ${qid(table)} (${columnSql}) VALUES (${valueSql});`;
}
...
try {
  ...
  for (const [table, columns] of baseTables) {
    ...
    if (table === "Tool") {
      ...
      pushRelationStatements(
        ...
        rows.flatMap((row) => [
          ...relationRows("ToolFeature", `${row.id}:en`, row.featuresEn, "value", { toolId: row.id, locale: "en" }),
          ...relationRows("ToolFeature", `${row.id}:zh`, row.featuresZh, "value", { toolId: row.id, locale: "zh" }),
        ]),
      );
      pushRelationStatements(
        lines,
  ...
  if (chunkSize > 0) {
    ...
    for (let index = 0; index < chunkCount; index += 1) {
      ...
      const chunkLines = [
        `-- Chunk ${index + 1}/${chunkCount} generated by scripts/export-postgres-to-d1.mjs`,
