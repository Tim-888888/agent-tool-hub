...
1. **Admin 用户识别机制** — RESOLVED: 使用 env var `ADMIN_GITHUB_IDS=userid1,userid2` 白名单方案。Plan 01 的 `src/lib/auth-helpers.ts` 实现了 `isAdmin()` 函数检查此环境变量。此方案简单可靠，无需额外 GitHub API 调用。后续可扩展为 GitHub team check。

2. **Next.js 16 proxy.ts 确切行为** — RESOLVED: Plan 01 不使用 middleware/proxy 做路由守卫。Admin 页面在 server component 中直接调用 `auth()` 检查权限并 `redirect("/")`。Plan 04 的 `src/app/admin/submissions/page.tsx` 使用此模式。实现时仍需参考 `node_modules/next/dist/docs/` 确认 Auth.js route handler 注册方式。

3. **PrismaAdapter 与 PrismaPg 的兼容性** — RESOLVED: Plan 01 中 `src/auth.ts` 直接传递 `prisma` 实例（PrismaClient）给 `PrismaAdapter(prisma)`。`db.ts` 导出的 `prisma` 是 PrismaClient 实例，PrismaAdapter 接受此类型。如果运行时发现不兼容，可临时为 auth models 创建独立的标准 PrismaClient。Plan 01 的 verify 步骤包含 `npx prisma generate` 验证 schema 正确性。

4. **Zod 是否已安装** — RESOLVED: Plan 01 Task 1 Step 1 明确包含 `npm install zod` 安装步骤。所有后续 Plan（02 review schema, 03 tag schema, 04 submit schema）均依赖此安装。
